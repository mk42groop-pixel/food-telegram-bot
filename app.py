...
# ДИАГНОСТИКА ТОКЕНОВ
def debug_tokens():
    """Показывает статус всех токенов для отладки"""
    ...
    all_ok = True
    for name, value in tokens_to_check.items():
        if value:
...
🎯 Сделайте паузу для ума - насладитесь умным десертом!"""
    ...
    try:
        ...
        try:
            ...
            if token and len(token) > 10:
                diagnostic_results['success'].append(f"Токен бота: {token[:10]}...")
                diagnostic_results['bot_status'] = 'token_ok'
            else:
                diagnostic_results['errors'].append("Токен бота не найден или невалиден")
...
if __name__ == '__main__':
    # Запускаем диагностику токенов при старте
    print("🔍 Запуск диагностики токенов...")
    debug_tokens()
